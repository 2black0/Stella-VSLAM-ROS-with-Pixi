...
class pose_logger {
public:
    bool init(const fs::path& output_path) {
        ...
        stream_.open(output_path.string(), std::ios::out | std::ios::trunc);
        if (!stream_.is_open()) {
            throw std::runtime_error("gagal membuka file log pose: " + output_path.string());
        }
        stream_ << "timestamp_s,tx,ty,tz,qw,qx,qy,qz,is_valid\n";
        return true;
    }

    void log(const double timestamp, const std::shared_ptr<stella_vslam::Mat44_t>& cam_pose_cw) {
        if (!stream_.is_open()) {
            return;
        }
        stream_ << std::fixed << std::setprecision(9) << timestamp << ",";
        if (cam_pose_cw) {
            const Eigen::Matrix3d rot = cam_pose_cw->block<3, 3>(0, 0);
            const Eigen::Vector3d trans = cam_pose_cw->block<3, 1>(0, 3);
            const Eigen::Quaterniond quat(rot);
            stream_ << trans.x() << "," << trans.y() << "," << trans.z() << ","
                    << quat.w() << "," << quat.x() << "," << quat.y() << "," << quat.z() << ",1\n";
        }
        else {
            stream_ << ",,,,,,,0\n";
        }
...
fs::path resolve_log_path(const std::optional<std::string>& raw_value_opt) {
    auto timestamp_str = [] {
    ...
    if (!raw_value_opt || raw_value_opt->empty()) {
        const auto timestamp = timestamp_str();
        return fs::path("code/log") / ("slam-" + timestamp) / "data-slam.csv";
    }

    const std::string& raw_value = *raw_value_opt;
    fs::path candidate(raw_value);

#ifdef _WIN32
    const bool ends_with_sep = !raw_value.empty() && (raw_value.back() == '\\' || raw_value.back() == '/');
#else
    const bool ends_with_sep = !raw_value.empty() && raw_value.back() == fs::path::preferred_separator;
#endif
    if (ends_with_sep) {
        candidate /= "data-slam.csv";
    }
    ...
    if (!candidate.has_filename() || candidate.filename().string() == "." || candidate.filename().string() == "..") {
        candidate /= "data-slam.csv";
    }
...
int mono_tracking(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    std::thread worker([&]() {
        while (keep_running.load()) {
            ...
            const auto cam_pose_cw = slam->feed_monocular_frame(frame, timestamp, mask);
            if (logger) {
                logger->log(timestamp, cam_pose_cw);
            }
...
int main(int argc, char* argv[]) {
    ...
    auto pose_log_output = op.add<popl::Value<std::string>>("", "log-output", "Path file CSV log pose (opsional)");
    try {
        op.parse(argc, argv);
    }
    catch (const std::exception& e) {
    ...
    pose_logger logger;
    pose_logger* logger_ptr = nullptr;
    std::optional<std::string> log_output_arg;
    if (pose_log_output->is_set()) {
        log_output_arg = pose_log_output->value();
    }
    const fs::path log_output_path = resolve_log_path(log_output_arg);
    try {
        logger.init(log_output_path);
        logger_ptr = &logger;
        spdlog::info("Logging SLAM poses to {}", log_output_path.string());
    }
    catch (const std::exception& e) {
        spdlog::error("Gagal menyiapkan file log pose: {}", e.what());
        return EXIT_FAILURE;
    }
