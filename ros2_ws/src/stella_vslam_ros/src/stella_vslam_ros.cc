...
system::system(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    // Align with common ROS image publishers (reliable, depth 10) to avoid mismatched QoS dropping frames
    custom_qos_.reliability = RMW_QOS_POLICY_RELIABILITY_RELIABLE;
    custom_qos_.history = RMW_QOS_POLICY_HISTORY_KEEP_LAST;
...
void system::setParams() {
    ...
    // QoS tuning: default RELIABLE depth 10; allow overriding via parameter
    custom_qos_.reliability = RMW_QOS_POLICY_RELIABILITY_RELIABLE;
    custom_qos_.history = RMW_QOS_POLICY_HISTORY_KEEP_LAST;
    custom_qos_.depth = 10;
    std::string qos_reliability = std::string("reliable");
    qos_reliability = node_->declare_parameter("qos_reliability", qos_reliability);
    if (qos_reliability == "best_effort" || qos_reliability == "best-effort") {
